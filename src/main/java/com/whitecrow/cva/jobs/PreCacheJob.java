package com.whitecrow.cva.jobs;
...
public class PreCacheJob {
    ...
    //重点用户
    private final List<Long> impUserList= Arrays.asList(1L);

    @Scheduled(cron = "0 31 0 * * *")
    public void doRecommendUser() {
        RLock lock = redissonClient.getLock("whitecrow:precacheJob:doRecommendUser:lock");
        try {
            if (lock.tryLock(0, 2000L, TimeUnit.MICROSECONDS)) {
                for (Long id : impUserList) {
                    ...
                    String redisKey = String.format("whitecrow:user:recommend:%s", id);
